...
Mu_d  <-   rnorm(24, 0.0073, 0.0073 / 10)
Sigma <-   rnorm(24, 0.023, 0.023 / 10)
Tau_nd <-  rnorm(24, 0.33, 0.33 / 10)
Sigma_d <- rnorm(24, 0.00015, 0.00015 / 10)
Beta <-    rnorm(24, 0.25, 0.25 / 10)
Theta <-   rnorm(24, 0.17, 0.17 / 10)
Weight <-  rnorm(24, 0.75, 0.75 / 10)
...
params <- cbind(Mu_d, Sigma, Tau_nd, Sigma_d, Beta, Theta, Weight)
...
recdat %>%
    ...
    geom_freqpoly(aes(x = rt, y = after_stat(count / nrow(recdat) * nrow(df)))) +
...
recdat %>%
    ...
    ggplot() + geom_line(aes(x = condition, y = high_conf, color = factor(choice))) +
    scale_color_discrete(labels = c("target", "distractor", "dud")) + labs(color = "choice") +
    scale_x_continuous(breaks = c(0, 27, 45, 63, 76, 86)) + xlab("Dud value") +
...
# source("fit.confidence.R")
df %>%
    dplyr::select(conf, chosenItem, val3) %>%
    rename(choice = chosenItem, condition = val3) %>%
    group_by(choice, condition) %>%
    summarise(mean_conf = mean(conf)) -> emp5
...
figure_mvn <- cowplot::plot_grid(p2 + guides(color = F, fill = F) + 
                                 ...
                                 p5 + guides(color = F),
                                 get_legend(p2),
                                 labels = c("a", "b", "c", "d"), label_size = 10, nrow = 3)
figure_mvn 