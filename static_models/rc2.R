...
for (sub in unique(dat$subj)) {
    ...
    f <- tryCatch(fit_rc2(d), error = function(e){f = cbind(NA, NA, NA)}) 
...
rc2 <- ggplot(acc) + geom_point(aes(x = val3, y = Accuracy, color = subj)) +
    geom_line(mapping = aes(x = val3, y = pred, color = subj)) + ylim(0.49, 0.9) + xlab("Condition") + 
    scale_x_continuous(breaks = c(0, 27, 45, 63, 76, 86)) +
    ggtitle("rc2")  + guides(color = guide_legend(title = NULL))
rc2

fits_rc2 <- fits
fits_rc2 <- mutate(fits_rc2, aic = dev + 4)